...
namespace Unreal.ObjectDumpToJson
{
    ...
    public unsafe class ObjectDump
    {
        ...
        {
            _context = context;
            _context._startupScanner.AddMainModuleScan("48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 41 56 41 57 48 83 EC 40 F6 81 ?? ?? ?? ?? 01", result =>
            {
        ...
        public unsafe void UPlayerInput_ProcessInputStack_Impl(UObjectBase* self, IntPtr InputComponentStack, float DeltaTime, byte bGamePaused)
        {
            ...
            {
                _context._logger.WriteLine($"Found {_context.g_objectArray->ObjObjects.NumElements} objects in GUObjectArray");
                ...
                for (int i = 0; i < _context.g_objectArray->ObjObjects.NumElements; i++)
                {
                    //if (i > _context.g_objectArray->ObjLastNonGCIndex) break;
                    //var target_item = _context.g_objectArray->ObjObjects.Objects[i];
                    var target_item = _context.g_objectArray->ObjObjects.GetObjectPtr(i);
                    var target_obj = target_item->Object;
                                                              ...
                                                              // (e.g The instance of PlayerInput obtained from UPlayerInput::ProcessInputStack)
        {
            ...
            IntPtr vtable = instance->VTable; // C++ vtable (don't have any other info on their names since Blueprints doesn't use them...)
            ...
            IntPtr ctor = type->ClassConstructor; // InternalCosntructor<UClassName> => UClassName::UClassName
            int class_size = type->Super.PropertiesSize;
            int class_align = type->Super.MinAlignment;

            UnrealClass uclass_exp = new UnrealClass(class_name, class_size, class_align, (uint)type->ClassFlags);
            ...
            UClass* super_type = (UClass*)((UStruct*)type)->SuperStruct;
            ...
            if (super_type != null)
            {
                ...
                uclass_exp.SuperType = (export.classes.TryGetValue(super_type_name, out UnrealClass prev_class))
                    ? prev_class : ExportClass(super_type->ClassDefaultObject);
            }
        ...
        public UnrealEnum ExportEnum(UEnum* type, int size)
        {
            ...
            // Get values
            List<UnrealEnumEntry> entries = new(type->Names.arr_num);
            for (int i = 0; i < type->Names.arr_num; i++)
            {
                UEnumEntry* curr_entry = &type->Names.allocator_instance[i];
        ...
        private UnrealField GetField(FProperty* curr_field)
        {
            ...
            var type = curr_field->Super.ClassPrivate; // get property type info
            string field_name = _context.g_namePool->GetString(((FField*)curr_field)->NamePrivate.pool_location);
            string type_name = _context.g_namePool->GetString(type->Name.pool_location);
            int offset = curr_field->Offset_Internal;
            int size = curr_field->ElementSize;
            ...
            switch (type_name)
            {
                ...
                case "SoftClassProperty": // TSoftClassPtr<UObject>
                    UClass* class_data = (type_name == "ClassProperty" || type_name == "SoftClassProperty")
                        ? ((FClassProperty*)curr_field)->MetaClass : ((FObjectProperty*)curr_field)->PropertyClass;
                    _context._logger.WriteLine($"Got ObjectProperty {_context.g_namePool->GetString(((FField*)curr_field)->NamePrivate.pool_location)} @ 0x{(nint)curr_field:X}");
                    if (class_data != null)
                    {
                        ...
                        UnrealClass tgt_class =
                            ...
                            ? prev_class : ExportClass(class_data->ClassDefaultObject)
                        ;
                ...
                case "ArrayProperty": // TArray<Type>
                    FProperty* inner_array = ((FArrayProperty*)curr_field)->Inner;
                    UnrealField inner_array_field = GetField(inner_array);
                    type_data = new UnrealFieldArrayType(type_name, inner_array_field);
                    break;
                case "SetProperty": // TSet<Type>
                    FProperty* inner_prop = ((FSetProperty*)curr_field)->ElementProp;
                ...
                case "MapProperty": // TMap<KeyType, ValueType>
                    FProperty* key_prop = ((FMapProperty*)curr_field)->KeyProp;
                    FProperty* value_prop = ((FMapProperty*)curr_field)->ValueProp;
            ...
            return new UnrealField(field_name, type_data, offset, size, curr_field->PropertyFlags);
        }
        ...
        private UnrealFunction ExportFunction(UFunction* func)
        {
            ...
            // Get parameters
            FProperty* curr_param = (FProperty*)((UStruct*)func)->ChildProperties;
            UnrealField? return_val = null;
            List<UnrealField> func_params = new();
            while (curr_param != null && func_params.Count < func->num_params && func->_super.Children != null)
            {
                ...
                curr_param = (FProperty*)curr_param->Super.Next;
            }
        ...
        private List<UnrealField> GetStructFields(UStruct* target)
        {
            ...
            FProperty* curr_field = (FProperty*)target->ChildProperties;
            while (curr_field != null)
            {
                fields.Add(GetField(curr_field));
                curr_field = (FProperty*)curr_field->Super.Next;
            }
            return fields;
        }
